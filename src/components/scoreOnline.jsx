...
export default function ScoreCardOnline() {
  ...
  const scoreDecision = (userRun, isWicket, opponentRun) => {
    ...
    if (!isWicket) {
      setPartnership((prev) => prev + (batting ? userRun : opponentRun));
  ...
  const updateTeam = async (
    ...
    const saveDataInDb = async (userTeamScore, opponentTeamScore) => {
      const { error } = await supabase
        .from("profiles")
        .update({
          onlineScore: userTeamScore,
        })
        .eq("id", profileId)
        .select()
        .single();
    ...
    await saveDataInDb(newUserTeam, newAiTeam);
